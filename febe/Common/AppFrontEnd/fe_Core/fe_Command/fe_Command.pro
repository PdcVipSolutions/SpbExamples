                ...
                then
                    ...
                    av(LanguageID,av(LanguageTitle,s(FlagId))) in LanguageIDTitle_List,
                        ...
                        if lngwCmd_F(CommandId,Command),! then
                            ...
                            Command:icon:=some(icon::createFromFile(string::format(@"febeAppData\pdcVipIcons\flags\flag-%.ico",FlagId)))
                        end if,
...
clauses
    noIconRender()=file::readBinary(@".\febeAppData\pdcVipIcons\status\image-missing.ico").
