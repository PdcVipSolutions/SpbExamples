...
clauses
    onMove(_Request,_Agency,_PlayerObj,string(polylineDomains::initialExternalMove_C),string(CellStr)):-
        ...
        invalidate(vpiDomains::rct((CellX-1)*Step,(CellY-1)*Step,(CellX-1)*Step+Step,(CellY-1)*Step+Step)),
        _IsSuccessful = vpi::processEvents().
    onMove(_Request,_Agency,PlayerObj,string(polylineDomains::ordinaryMove_C),string(CellStr)):-
...
clauses
    ...
    invalidateCell(c(X,Y)):-
        getBoardDrawParameters(Step,_WidthPxl,_HeightPxl),
        invalidate(vpiDomains::rct(Step*(X-1),Step*(Y-1),Step*(X-1)+Step,Step*(Y-1)+Step)).
...
clauses
    modifyLayout():-
        ...
        vpiDomains::pnt(WidthPxl,HeightPxl)=DlgContainer:pntUnit2Pixel(pnt(ActualWidth,ActualHeight)),
        ...
        vpiDomains::pnt(BoardWidthDlgbu,BoardHeightDlgbu)=DlgContainer:pntPixel2Unit(pnt(BoardWidth,BoardHeight)),
        ...
        setRect(vpiDomains::rct(NewHorPosition,NewVerPosition,NewHorPosition+BoardWidthDlgbu,NewVerPosition+BoardHeightDlgbu)).
...
clauses
    onPaint(_Source, Rectangle, GDIObject):-
        ...
        GDIObject:clear(Rectangle,vpiDomains::color_LtGray),
        GDIObject:setPen(vpiDomains::pen(1,ps_Solid,vpiDomains::color_gray)),
...
clauses
    getBoardDrawParameters(Step,WidthPxl,HeightPxl):-
        ...
        vpiDomains::pnt(WidthPxl,HeightPxl)=ParentDialog:pntUnit2Pixel(pnt(Width,Height)),
...
clauses
    ...
    onChar(_Source, _AnyOther, _ShiftControlAlt) =  window::defaultCharHandling.
