...
clauses
    ...
    setModal(_BackFocusControl):-
        exception::raise_User("Extra alternative!").
...
clauses
    getCtrlLocation()=console_native::coord(Xcont+Xctl,Ycont+Yctl):-
        location_V=console_native::coord(Xctl,Yctl),
        try
            Container=getConsoleControlContainer(),
            ContainerControl=convert(consoleControl,Container),
            console_native::coord(Xcont,Ycont)=ContainerControl:getCtrlLocation()
        catch _TraceID do
            Xcont=0,
            Ycont=0
        end try.
...
clauses
    getConsoleRoot1(Control)=ControlContainerRoot:-
        ...
        ControlContainerEventSource=tryConvert(consoleEventSource,ControlContainer),
        ControlContainerRoot=tryConvert(consoleControlContainer,ControlContainerEventSource),
        !.
    getConsoleRoot1(Control)=ControlContainerRoot:-
        ControlContainer=Control:getConsoleControlContainer(),
        hasDomain(consoleControlContainer,ControlContainer),
...
clauses
    ...
    onSystemEvent(_Request,_NotiAgency,EventCousedObject,string(consoleEventSource::mouseDblClickEventID_C),string(EventData)):-
        ...
        exception::raise_user("Catch Mouse Event Exception").
...
clauses
    handleMouseClick(EventCousedObject,EventData)=console_native::coord(Xclick,Yclick):-
        ...
        console_native::coord(Xctl,Yctl)=getCtrlLocation(),
