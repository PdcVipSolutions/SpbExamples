...
clauses
    stopGame():-
        try
        	game_V:humanInterface_V:unsubscribe(moveListener)
        catch _TraceID do
        	succeed
        end try.
...
clauses
    onNotification(_Object1,_Object2,_Object3,object(This),none):- % Invites to make Move
        game_V:humanInterface_V:subscribe(moveListener),
        game_V:humanInterface_V:showStatus(polylineDomains::humanMove(This)),
        !.
